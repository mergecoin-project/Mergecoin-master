...
# **Mergecoin-Core (MGC) v1.0**

[![Build Status](https://travis-ci.org/Mergecoin/ion.svg?branch=master)](https://travis-ci.org/Mergecoin/ion)

Mergecoin Integration/Staging Tree
================================

**Copyright (c) 2016-2017 Mergecoin**
...
Mergecoin is a digital currency that enables instant payments to anyone, anywhere in the world. Mergecoin uses peer-to-peer technology over ClearNet to operate with no central authority (centralisation): managing transactions and issuing currency (ION) are carried out collectively by the Mergecoin network. Mergecoin is the name of open source software which enables the use of the currency Mergecoin.
...
UNIX BUILD NOTES
====================
Some notes on how to build Mergecoin in Unix.

Note
---------------------
Always use absolute paths to configure and compile Mergecoin and the dependencies,
...
This will build Mergecoin-Qt as well if the dependencies are met.
...
memory available when compiling Mergecoin Core. With 512MB of memory or less
...
If you want to build Mergecoin-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a Mergecoin-Qt executable will be
built by default.

Notes
-----
The release is built with GCC and then "strip Mergecoind" to strip the debug
...
Security
--------
To help make your Mergecoin installation more secure by making certain attacks impossible to
