...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Mergecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Mergecoin
    // Mac: ~/Library/Application Support/Mergecoin
    // Unix: ~/.Mergecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mergecoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Mergecoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty Mergecoin.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Mergecoind.pid"));
